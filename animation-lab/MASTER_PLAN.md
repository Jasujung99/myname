...
1. **입자 시스템 시뮬레이션**: 입자들 사이에 중력, 전기력 등 상호작용을 설정하고, 질량, 전하 같은 파라미터를 Vega로 조작하여 움직임을 관찰합니다.
...
2. **포식자-피식자 모델**: 생태계에서 여러 개체군(토끼, 여우, 풀 등)의 수가 서로 어떻게 영향을 주며 주기적으로 변하는지 시뮬레이션합니다.
...
1. **경로 탐색 알고리즘**: A*, 다익스트라 등 길 찾기 알고리즘이 미로 속에서 최단 경로를 탐색하는 과정을 단계별로 시각화합니다.
...
5. **음성/터치 제어**: 마이크 입력(음높이, 음량)이나 터치 제스처에 따라 그래픽이 실시간으로 반응하는 인터랙티브 아트를 만듭니다.
