...
### 📐 수학 (Mathematics)
- **[기하학적 변환](animation-lab/experiments/math/geometric-transforms.html)** - 행렬의 마법으로 도형 변환하기 ✨
  - **5가지 도형**: 삼각형, 사각형, 오각형, 별, 화살표로 다양한 변환 체험
  - **4가지 변환**: 회전, 크기조절, 전단, 반사를 실시간으로 조합
  - **변환 큐 시스템**: 여러 변환을 순서대로 쌓아 복잡한 변환 생성
  - **실시간 행렬 시각화**: 변환 행렬과 합성 행렬을 실시간으로 확인
  - **애니메이션 효과**: 부드러운 변환 애니메이션과 파티클 효과
  - **기저 벡터 표시**: 변환된 좌표계를 시각적으로 이해
